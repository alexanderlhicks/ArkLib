...
3. `U0a`: once the active fixed-fraction `U0` work integrates, add the manuscript's adaptive
   `ρ₀=δ(1-δ)/2`, `K=max{k,ceil(ρ₀n)}` geometry in a separate file. Preserve the
   fixed-fraction theorem as an alternate qualitative route.
4. After the `S0.1` synchronization checkpoint, `O0-O1` audit support: independently develop the
   continuous and discrete `169/25` certificates in non-overlapping helper files. Add a new
   asymmetric-band index with a lower and upper ordinary-degree edge; do not alter the existing
   proof-facing exact index. Final composition still waits for `C0`, `I4`, and `U0a`.
5. `N0`, `N2a`, or `N3`: formalize the exact-capacity maximum, the basic characteristic boundary,
   or the universal agreement bounds. These are useful independent checks, but they do not replace
   a critical-path node. Start `N2b` only after the asymmetric-band support interface is frozen.
