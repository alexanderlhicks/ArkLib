...
def discr_y {F : Type} [CommRing F] (f : F[X][Y]) : F[X] :=
  ...
  if hf : 0 < f.degree then
    Classical.choose (resultant_is_divisible_by_leadingCoeff f hf)
  else
    0

lemma discr_y_spec {F : Type} [CommRing F] (f : F[X][Y]) (hf : 0 < f.degree) :
    Polynomial.resultant f f.derivative f.natDegree (f.natDegree - 1) =
      f.leadingCoeff * discr_y f := by
  classical
  have h := Classical.choose_spec (resultant_is_divisible_by_leadingCoeff f hf)
  simpa [discr_y, hf, dif_pos hf] using h
