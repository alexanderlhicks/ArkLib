...
/-!
# Uniform finite donor interpolation certificates

This module strengthens the finite-rate-bin donor package with the strict finite comparison
...
The finite mesh, exact interpolation index, and uniform assembly here are ArkLib-specific.
-/

namespace ReedSolomon
namespace AllRateListDecoding
namespace DonorFiniteCertificate
...
/-- Every bin has one eventual order threshold satisfying both the already-landed scalar donor
...
theorem exists_bin_order_threshold_with_shell {delta : ℝ}
  ...
  obtain ⟨dScalar, hScalar⟩ :=
    DonorRateBins.exists_bin_order_threshold hdelta hdeltaOne i
...
/-- The finite certificate needs more room than the rectangular donor package: `2d+3 < K`
...
/-- The strengthened block threshold retains all donor side conditions and leaves a full unit
...
degree strictly below the block length.  This is a separate bound from the donor's rectangular
degree budget `B < n`. -/
...
/-- A covered donor bin carrying the exact finite rank comparison and both characteristic-sized
degree bounds needed by later interpolation and root-counting consumers. -/
structure FiniteCoveredBinCertificate (F : Type*) [Field F]
    (delta : ℝ) (d n q k : ℕ) (i : Fin (binCount delta)) : Prop
    extends CoveredBinCertificate delta d n q k i where
...
theorem FiniteCoveredBinCertificate.toExactFiniteCertificate
    {F : Type*} [Field F] {delta : ℝ} {d n q k : ℕ}
    {i : Fin (binCount delta)}
    (certificate : FiniteCoveredBinCertificate F delta d n q k i) :
...
theorem exists_uniform_finite_covered_bin_certificates.{u} {delta : ℝ}
    ...
    ∃ d N : ℕ, 2 ≤ d ∧ ∀ n q k : ℕ, N ≤ n → n ≤ q →
      ...
      ∃ i : Fin (binCount delta), ∀ (F : Type u) [Field F],
        FiniteCoveredBinCertificate F delta d n q k i := by
  ...
  let certificate : BinCertificate delta d n q i := {
    orderAtLeastTwo := hd
  ...
  have hArithmetic :
    ...
    apply donorScalar_lt_globalRectangle htheta.1 (by omega : 0 < d) hnPos
  ...
  refine ⟨i, fun F _ ↦ {
    toCoveredBinCertificate := {
      toBinCertificate := certificate
      agreement_mem := hepsilon
      slack_mem := htheta
      rateIdentity := hIdentity
      messageDim_le_ambient := hMessage
      donorAgreement_le_requested := hAgreementRequested
    }
