...
/-!
# Uniform hidden-derivative constructions from donor certificates

The finite donor certificate and the actual local-rank bound give a nonzero exact interpolant.
Its donor agreement threshold is at most the requested `k + ceil(delta * n)`: this relaxes the
weighted-degree bound while retaining the sharper donor support bound for the characteristic
...
executable decoder or operation bound. The imported donor estimates retain their permission and
provenance records in `DonorFiniteCertificate.lean`.

## References

* [Dao, Kominers, Thaler, and Zheng, *Reed-Solomon List Decoding up to Capacity at Every
  Rate*][DKTZ26], source revision `9e4d6488ead94be47cca69e5be915b5667143b66`, global
  interpolation principle and uniform all-rate construction.
-/

namespace ReedSolomon.AllRateListDecoding

open HiddenDerivative DonorRateBins DonorFiniteCertificate

noncomputable section

/-- A finite donor certificate produces an actual construction at the requested agreement
threshold. The contact budget here is for that requested threshold, rather than the smaller donor
threshold. Original messages retain their dimension `k`, inside the donor ambient space. -/
theorem DonorFiniteCertificate.FiniteCoveredBinCertificate.nonempty_construction
    {delta : ℝ} {d n q k : ℕ} {i : Fin (RateCover.binCount delta)} [Fact q.Prime]
    (certificate : FiniteCoveredBinCertificate (ZMod q) delta d n q k i)
    (hContact : d ^ 3 * agreementThreshold delta n k ≤ q ^ 2)
    (domain : Fin n ↪ ZMod q) (received : Fin n → ZMod q) :
    Nonempty (HiddenDerivativeConstruction (k := k) (A := agreementThreshold delta n k)
  ...
  · exact (differentialWeightedDegree_lt_of_mem_exactInterpolationSpace
      hBudget hdD hQSpace).trans_le
        (Nat.mul_le_mul_left (d ^ 3) certificate.donorAgreement_le_requested)
...
the characteristic budget still uses the donor exact interpolation space. -/
theorem uniform_hidden_derivative_construction : UniformHiddenDerivativeConstructionStatement := by
  ...
  apply (hCertificate (ZMod q)).nonempty_construction _ domain received
  calc
