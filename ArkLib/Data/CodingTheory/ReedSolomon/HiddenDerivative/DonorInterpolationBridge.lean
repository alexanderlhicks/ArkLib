...
/-!
# Donor interpolation dimension bridge

This file connects the exact finite count used by the hidden-derivative interpolation space to
the coarse numerical estimates in the donor all-rate argument.  In particular, it bounds the
...
/-- The exhibited-kernel residual sum has the donor's coarse `4 d⁸` upper bound. -/
...
/-- Exponent in the donor's shell ratio. -/
...
/-- Convert the donor's real shell and width estimates into the strict natural-number rectangle
comparison. -/
theorem donorScalar_lt_globalRectangle {theta : ℝ} {d K H R n : ℕ}
...
/-- The donor shell and scalar hypotheses imply the exact finite-dimensional inequality consumed
by interpolation-kernel extraction. -/
theorem donorShell_lt_finrank_exactInterpolationSpace
  ...
  exact donorScalar_lt_globalRectangle htheta hd hn hH hR hcompare
